...
public class BezierCurve
{
	...
	public BezierCurve(Vector3[] points, float rotationP1 = 0f, float rotationP4 = 0f)
    {
        this.points = points;

		this.rotationP1 = rotationP1;
		this.rotationP4 = rotationP4;
    ...
    void GenerateSamples()
    {
        ...
        List<float> samples = new List<float>(s) { 0 };
		step = 1.0f / (float)s;
    ...
    public OrientedPoint GetOrientedPoint(float t)
    {
		...
		float rotation = Mathf.Lerp(rotationP1, rotationP4, t);

        Vector3 point = GetPoint(t, out tangent, out normal, out orientation);

		orientation *= Quaternion.Euler(0, 0, rotation);

		return new OrientedPoint(point, orientation, sampledLengths.Sample(t) * step);
    }
